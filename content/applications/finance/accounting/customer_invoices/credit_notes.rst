...
go to :menuselection:`Accounting --> Customers --> Invoices`, open the relevant :guilabel:`Invoice`,
and click :guilabel:`Debit Note`. Then, follow these steps:
...
.. tip::
   To create a debit note from the invoice form view, select the desired invoice(s), click
...
:guilabel:`Vendor bill`, and click :guilabel:`Debit Note`.

.. tip::
   To create a debit note from the vendor bill form view, select the desired vendor bill(s), click
   :icon:`fa-cog` :guilabel:`Actions` and select :guilabel:`Create Debit Note`.
