...
values <search/values>`, or click the :icon:`fa-caret-down` (**dropdown**) icon to access the
:ref:`Filter <search/filters>`, :ref:`Group By <search/group>`, and :ref:`Favorites
...
.. example::
   ...
   *Mitchell Admin* is the salesperson on the *Sales Analysis* report (:menuselection:`Sales app -->
   Reporting --> Sales`), search for `Mitch`, and click the :icon:`fa-caret-right` (**submenu**)
   icon next to :guilabel:`Search Salesperson for: Mitch`, and select :guilabel:`Mitchell Admin`.
...
.. note::
   ...
   selected (without selecting the :icon:`fa-caret-right` (**submenu**), *all* records containing
...
Modify the default selection of records by clicking the :icon:`fa-caret-down` (**dropdown**) from
the search bar, and selecting one (or several) *preconfigured filters* from the :guilabel:`Filters`
drop-down menu.
...
.. example::
   ...
   :icon:`fa-times` (**cancel**), then select the :menuselection:`Order Date --> 2024` filter.
...
custom filter. To do so, click the :icon:`fa-caret-down` (**dropdown**) icon in the search bar, then
...
#. The first inline field is the *field name* to filter by. Some fields have refined parameters that
   are nested within another field. These fields have a :icon:`fa-caret-right` (**submenu**) icon
...
#. :icon:`fa-plus` (**plus**): Adds a new rule below the existing rule.
#. :icon:`fa-sitemap` (**node**): Adds a new group of rules below the existing rule, with the
...
#. :icon:`fa-trash` (**delete**): Deletes the node. If a branch node is deleted, all children of
...
groups*. To do so, click the :icon:`fa-caret-down` (**dropdown**) icon in the search bar, then
select one of the :guilabel:`Group By` options from the drop-down menu.
...
To save the current view as a favorite, click the :icon:`fa-caret-down` (**dropdown**) icon in the
search bar, then select the :guilabel:`Save current search` drop-down menu to display the following
options:
...
Saved favorites can be accessed by clicking the :icon:`fa-trash` (**delete**) icon in the search
bar, then selecting the saved filter in the :guilabel:`Favorites` drop-down menu. To remove a saved
favorite, click the :icon:`fa-trash` (**delete**) icon next to the favorited search.
